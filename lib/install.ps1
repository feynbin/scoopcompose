    ...
    if (!(Test-Path $cached) -or !$use_cache) {
        ...
        if ($proxy){
            ...
            if ($url -like 'https://github.com*') {
                $url = "https://git.feynbin.cn/https://github.com$($url -replace 'https://github.com', '')"
            }
        }
        ...
        if ((Test-Path $data.$url.source) -and -not((Test-Path "$($data.$url.source).aria2") -or (Test-Path $urlstxt)) -and $use_cache) {
            ...
            try {
                ...
                if ($proxy){
                    ...
                    $try_url = handle_special_urls $url
                    if ($try_url -like 'https://github.com*') {
                            $try_url = "https://git.feynbin.cn/https://github.com$($try_url -replace 'https://github.com', '')"
                    }
                }                
            } catch {
...
function Invoke-Download ($url, $to, $cookies, $progress) {
    ...
    try {
        ...
        Write-Host $newUrl $to $cookies $progress
        return
    }
